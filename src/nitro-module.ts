...
async function writeHandlerFile({
  ...
  // Session mode swaps the stateless handler for the server-side-session one (owned history + streaming).
  const handlerImport = sessionFile
    ? `import { buildToolSet, createSessionChatHandler, defaultInvoke, resolveChatConfig, type ToolRoute } from "@elumixor/nitro-bot";`
    : `import { buildToolSet, createChatHandler, defaultInvoke, resolveChatConfig, type ToolRoute } from "@elumixor/nitro-bot";`;
  const sessionImport = sessionFile
    ? `import chatSession from ${JSON.stringify(sessionFile.replace(/\.tsx?$/, ""))};\n`
    : "";
  const defaultExport = sessionFile
    ? `export default createSessionChatHandler({
  session: chatSession,
  tools: toolRoutes,
...
async function writeBotPlugin({ buildDir, bot }: { buildDir: string; bot: DiscoveredBot }): Promise<string> {
  ...
  const commandImports = bot.commandFiles
    ...
    .join("\n");
  const subagentImports = bot.subagentFiles
    .map((f, i) => `import subagent_${i} from ${JSON.stringify(stripExt(f))};`)
    .join("\n");
  ...
  const commandArr = bot.commandFiles
    ...
    .join(", ");
  // Subagent name defaults to the file name (time.ts → "time"); explicit `name` in the file wins.
  const subagentArr = bot.subagentFiles
    .map((f, i) => `{ ...subagent_${i}, name: subagent_${i}.name || ${JSON.stringify(commandNameFromFile(f))} }`)
    .join(", ");
